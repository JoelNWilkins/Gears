...
if __name__ == "__main__":
    ...
    # Display the path of the data file
    print("XLS files in directory {}".format(path))

    # Display all the names of the xls files in the data directory
    for i in range(len(xlsFiles)):
        print("{}: {}".format(i + 1, xlsFiles[i]))

    # Ask the user for the first file to open
    run = True
    while run:
        try:
    ...
    # Ask the user for the second file to open
    run = True
    while run:
        try:
    ...
    # Run the model
    main([xlsFiles[number1 - 1], xlsFiles[number2 - 1]])
